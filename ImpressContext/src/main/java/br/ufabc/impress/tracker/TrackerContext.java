...
public class TrackerContext {
	...
	private static String address_db = "jdbc:postgresql://localhost/impress_2?user=postgres&password=postgres";
	...
	private int saveDb(String context_sequence, int context_count,
		...
		try {
			...
			String sql = "INSERT INTO context_count (context_sequence, context_count, context_registered) values (?, ?, ?)";
	...
	private void updateDb(String context_sequence, int context_count,
		...
		try {
			ResultSet rs = stm
					.executeQuery("select * from context_count where id_context_count = '"
							+ index + "'");
		...
		try {
			String sql = "UPDATE context_count SET context_sequence = ? , context_count = ?, context_registered = ? WHERE id_context_count = '"
					+ index + "'";
				...
				for (j = 0; j < m; j++) {
					...
					for (d = 0; d < tttam; d++) {
						if (lista13.get(d) == null) {
							// System.out.println("LIGAÇÃO COM SENSOR:  " +
							// lista14.get(d));
