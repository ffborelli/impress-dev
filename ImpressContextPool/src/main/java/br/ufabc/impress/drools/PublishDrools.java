			...
			//if (messages.isEmpty() == false) {
				...
				for (int i = 0; i < messages.size(); i++) {
					...
					//if (!messages.get(i).equalsIgnoreCase("-1")) {
						...
						String resource = EvalUtil.getParameter("resource", messages.get(i), ";", "=");
						...
						r = this.getResourceFacade().find(Integer.parseInt(resource));
						...
						Rule rule = this.getRuleFacade().find(Integer.parseInt(resource));
...
//						t.start();
						System.out.println("MESSAGE HAS BEEN SENT -- > " + messages.get(i));
						MqttPublishModel model = new MqttPublishModel(r.getMqttAddress(), "ivan"+r.getId(), "/ivan/publish", messages.get(i));
						MqttIvan.putMessageMqttPublish(model);
					//}
				//}
